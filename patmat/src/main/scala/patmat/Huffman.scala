...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def translate(auxTree: CodeTree, bits: List[Bit]): List[Char] = auxTree match {
      case Leaf(c, _) => if (bits.isEmpty) List(c) else c :: translate(tree,bits)
      case Fork(left, right, _, _) => if (bits.head == 0) translate(left, bits.tail) else translate(right, bits.tail)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def translate(tree: CodeTree)(c: Char): List[Bit] = tree match{
      case Leaf(c,_) => Nil
      case Fork(left, right, _, _) => if (chars(left).contains(c)) 0 :: translate(left)(c) else 1 :: translate(right)(c)
    }
    text.flatMap(translate(tree))
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    if (table.isEmpty) Nil
      ...
      if (table.head._1 == 0) table.head._2
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(b: Bit)(pair: (Char, List[Bit])): (Char, List[Bit]) = (pair._1, b :: pair._2)
